/**
 * Onboard Command
 * 
 * Founder agent initialization - reads project documents.
 */

import { readFile } from 'fs/promises';
import { join } from 'path';
import { Command, CommandContext, CommandResult, CommandType, ResponseBuilder } from './framework.js';

const FILES_TO_READ = [
  { path: 'README.md', desc: '项目概览', priority: 'high', maxLength: 2000 },
  { path: 'AGENTS.md', desc: '项目指南', priority: 'high', maxLength: 2000 },
  { path: 'package.json', desc: '依赖配置', priority: 'medium', maxLength: 1000 },
  { path: 'docs/architecture/architecture-overview.md', desc: '架构文档', priority: 'high', maxLength: 1500 },
];

export class OnboardCommand extends Command {
  constructor() {
    super({
      name: 'onboard',
      description: '创始者Agent初始化：读取项目文档了解代码库',
      usage: '/onboard',
    });
  }

  async execute(context: CommandContext): Promise<CommandResult> {
    const { agent, onProgress } = context;

    // Only for founder template
    if (agent.ai.templateId !== 'founder') {
      return {
        type: CommandType.ONBOARD,
        success: false,
        response: `⚠️ /onboard 命令专为创始者Agent设计。当前模板: ${agent.ai.templateId}`,
      };
    }

    try {
      const cwd = process.cwd();

      // Send initial progress
      if (onProgress) {
        await onProgress(
          `🚀 开始项目初始化\n` +
          `Agent: ${agent.name}\n` +
          `共 ${FILES_TO_READ.length} 个文件\n\n` +
          `开始学习...`
        );
        await sleep(500);
      }

      let projectContext = ResponseBuilder.create()
        .title('📋 项目交接文档')
        .line(`Agent: ${agent.name}`)
        .line(`工作目录: ${cwd}`)
        .line(`初始化时间: ${new Date().toLocaleString()}`)
        .build();

      let loadedCount = 0;
      let failedCount = 0;

      // Read each file
      for (let i = 0; i < FILES_TO_READ.length; i++) {
        const file = FILES_TO_READ[i];
        const progress = `[${i + 1}/${FILES_TO_READ.length}]`;

        try {
          if (onProgress) {
            await onProgress(`${progress} 📖 正在学习: ${file.desc} (${file.path})...`);
          }

          const content = await readFile(join(cwd, file.path), 'utf-8');
          loadedCount++;

          const maxLen = file.maxLength || 1500;
          const truncated = content.length > maxLen
            ? content.substring(0, maxLen) + '\n\n...[内容已截断，完整内容请查看文件]'
            : content;

          projectContext += `\n\n---\n## ${file.desc} (${file.path})\n\n${truncated}`;

          // Report progress
          const remaining = FILES_TO_READ.length - i - 1;
          if (onProgress) {
            const statusMsg = remaining > 0
              ? `${progress} ✅ 已完成: ${file.desc}（还剩 ${remaining} 个文件）`
              : `${progress} ✅ 已完成: ${file.desc}`;
            await onProgress(statusMsg);
          }
        } catch (err) {
          failedCount++;
          projectContext += `\n\n---\n## ${file.desc} (${file.path})\n\n⚠️ 无法读取: ${(err as Error).message}`;

          if (onProgress) {
            await onProgress(`${progress} ⚠️ 跳过: ${file.desc}（读取失败）`);
          }
        }

        await sleep(800);
      }

      // Final summary
      projectContext += ResponseBuilder.create()
        .line()
        .line('---')
        .line()
        .line('✅ 项目交接完成！')
        .line()
        .line('作为创始者Agent，你的职责：')
        .numbered([
          '熟悉项目架构和技术栈',
          '理解TDD开发流程',
          '维护代码质量和文档',
          '协助项目演进和重构',
        ])
        .line()
        .line('可以使用 /status 查看状态，开始维护项目吧！')
        .build();

      await sleep(1000);

      if (onProgress) {
        await onProgress(
          `🎉 初始化完成！\n` +
          `成功: ${loadedCount} | 失败: ${failedCount}\n` +
          `正在发送完整文档...`
        );
        await sleep(500);
      }

      return {
        type: CommandType.ONBOARD,
        success: true,
        response: projectContext,
        data: {
          agentId: agent.id,
          workspace: cwd,
          filesLoaded: loadedCount,
          filesFailed: failedCount,
          totalFiles: FILES_TO_READ.length,
        },
      };
    } catch (error) {
      return ResponseBuilder.error('项目初始化失败: ' + (error as Error).message, undefined, CommandType.ONBOARD);
    }
  }
}

function sleep(ms: number): Promise<void> {
  return new Promise(r => setTimeout(r, ms));
}
